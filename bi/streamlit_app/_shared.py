...
def _load_dimensions() -> dict:
    """Charge dim_marque et dim_pays et renvoie des maps clé->nom si trouvées."""
...
def load_data() -> pd.DataFrame:
    """Charge le fact + enrichit avec noms de marques/pays si disponibles.
       GARANTIT la présence des colonnes 'marque' et 'pays' (fallback sur *_key)."""
...
def sidebar_filters(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    with st.sidebar:
        ...
        sel_years = st.multiselect("Années", years, default=years) if years else []
        ...
        min_dt = pd.to_datetime(df["month_key"].min()) if "month_key" in df.columns else pd.Timestamp("2018-01-01")
        max_dt = pd.to_datetime(df["month_key"].max()) if "month_key" in df.columns else pd.Timestamp("2024-12-01")
    ...
    if "month_key" in df.columns:
        mask &= df["month_key"].between(from_dt, to_dt)
    if "year" in df.columns and sel_years:
        mask &= df["year"].isin(sel_years)
    if "marque" in df.columns and sel_marques:
        mask &= df["marque"].isin(sel_marques)
    if "pays" in df.columns and sel_pays:
...
def ytd(df: pd.DataFrame, value_col="ca") -> Dict[str, float | None]:
    if df.empty or "month_key" not in df or "year" not in df:
