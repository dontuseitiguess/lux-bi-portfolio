...
try:
    ...
    prophet_ok = False
    st.info("Prophet indisponible sur cet environnement.")
...
if prophet_ok and len(ts) >= 12:
    ...
    try:
        ...
        fig_fc = go.Figure()
        fig_fc.add_trace(go.Scatter(x=ts["ds"], y=ts["y"], mode="lines", name="Historique"))
        fig_fc.add_trace(go.Scatter(x=forecast_df["ds"], y=forecast_df["yhat_upper"],
                                    mode="lines", name="IC sup", line=dict(width=0), showlegend=False))
        fig_fc.add_trace(go.Scatter(x=forecast_df["ds"], y=forecast_df["yhat_lower"],
                                    fill="tonexty", mode="lines", name="IC", line=dict(width=0)))
        fig_fc.add_trace(go.Scatter(x=forecast_df["ds"], y=forecast_df["yhat"],
    ...
    try:
        ...
        st.markdown(
            "- YoY {} -> {} : {}\n- CAGR historique {} -> {} : {}\n- CAGR prevu : {}".format(
                ...
                y_first,
                last_year,
                (fmt_pct(cagr_hist_val) + "/an") if cagr_hist_val is not None else "n/a",
                (fmt_pct(cagr_fc_val) + "/an") if cagr_fc_val is not None else "n/a",
            )
        )

        report_md = (
            "## Tendances & Prévisions — Résumé executif\n\n"
            f"- Historique : serie mensuelle du CA de {s['ds'].min().date()} à {s['ds'].max().date()} ({len(s)} points).\n"
            f"- YoY {last_year-1}->{last_year} : {fmt_pct(yoy_val)}.\n"
            f"- CAGR historique {y_first}->{last_year} : {(fmt_pct(cagr_hist_val) + '/an') if cagr_hist_val is not None else 'n/a'}.\n"
            f"- Prévision : Prophet (saisonnalite annuelle), sensibilite {cp_scale}.\n"
            f"- CAGR prevu (fin d'horizon) : {(fmt_pct(cagr_fc_val) + '/an') if cagr_fc_val is not None else 'n/a'}.\n"
...
gt = load_trends_csv()
if gt is None:
    st.info("Aucun CSV Google Trends trouve. Place 'google_trends.csv' dans data/processed/ ou data/raw/ (colonnes: date, topic, score).")
else:
    ...
    if not (date_col and topic_col and score_col):
        ...
        try:
            ...
            df_corr = series.join(pivot, how="inner")
            ...
            if corrs.empty:
                st.info("Pas assez de recouvrement temporel pour calculer la correlation.")
            else:
                ...
                fig_corr = px.bar(
                    corrs_sorted, x="corr_Pearson", y="topic", orientation="h",
                    color="signe",
